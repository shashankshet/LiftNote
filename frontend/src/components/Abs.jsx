...
const Abs = () => {
  ...
  useEffect(() => {
    const storedLogs = JSON.parse(localStorage.getItem("absWorkoutLogs")) || [];
    setWorkoutLogs(storedLogs);
  }, []);

  const absExercises = [
    { name: "Crunches", image: "static/images/crunches.png" },
    { name: "Leg Raises", image: "static/images/legraises.png" },
    { name: "Russian Twists", image: "static/images/russiantwists.png" },
  ];
  ...
  const handleSave = () => {
    ...
    localStorage.setItem("absWorkoutLogs", JSON.stringify(updatedLogs));
    handleCloseModal();
  };
  ...
  const generateReport = () => {
    ...
    toPng(reportRef.current)
      .then((dataUrl) => {
        ...
        link.download = "abs_workout_report.png";
        link.href = dataUrl;
        link.click();
      })
      .catch((err) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-4 sm:p-6">
      <h1 className="text-2xl font-bold mb-6 mt-4 sm:mt-8">Abs Exercises</h1>
