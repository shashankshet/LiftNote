...
const Triceps = () => {
  ...
  useEffect(() => {
    const storedLogs =
      JSON.parse(localStorage.getItem("tricepsWorkoutLogs")) || [];
    setWorkoutLogs(storedLogs);
  }, []);

  const tricepsExercises = [
    { name: "Tricep Dips", image: "static/images/tricepdips.png" },
    { name: "Skull Crushers", image: "static/images/skullcrushers.png" },
    { name: "Tricep Pushdowns", image: "static/images/triceppushdown.png" },
    {
      name: "Overhead Tricep Extension",
      image: "static/images/overheadtricpeextention.png",
    },
  ];
  ...
  const handleSave = () => {
    ...
    localStorage.setItem("tricepsWorkoutLogs", JSON.stringify(updatedLogs));
    handleCloseModal();
  };
  ...
  const generateReport = () => {
    ...
    toPng(reportRef.current)
      .then((dataUrl) => {
        ...
        link.download = "triceps_workout_report.png";
        link.href = dataUrl;
        link.click();
      })
      .catch((err) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-4 sm:p-6">
      <h1 className="text-2xl font-bold mb-6 mt-4 sm:mt-8">
        Triceps Exercises
      </h1>
