...
const Legs = () => {
  ...
  useEffect(() => {
    const storedLogs =
      JSON.parse(localStorage.getItem("legsWorkoutLogs")) || [];
    setWorkoutLogs(storedLogs);
  }, []);

  const legsExercises = [
    { name: "Squats", image: "static/images/squats.png" },
    { name: "Lunges", image: "static/images/lunges.png" },
    { name: "Leg Press", image: "static/images/legpress.png" },
    { name: "Calf Raises", image: "static/images/calfraises.png" },
  ];
  ...
  const handleSave = () => {
    ...
    localStorage.setItem("legsWorkoutLogs", JSON.stringify(updatedLogs));
    handleCloseModal();
  };
  ...
  const generateReport = () => {
    ...
    toPng(reportRef.current)
      .then((dataUrl) => {
        ...
        link.download = "legs_workout_report.png";
        link.href = dataUrl;
        link.click();
      })
      .catch((err) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-4 sm:p-6">
      <h1 className="text-2xl font-bold mb-6 mt-4 sm:mt-8">Legs Exercises</h1>
