...
const Biceps = () => {
  ...
  useEffect(() => {
    const storedLogs =
      JSON.parse(localStorage.getItem("bicepsWorkoutLogs")) || [];
    setWorkoutLogs(storedLogs);
  }, []);

  const bicepsExercises = [
    { name: "Bicep Curls", image: "static/images/bicepcurl.png" },
    { name: "Hammer Curls", image: "static/images/hammercurl.png" },
    {
      name: "Concentration Curls",
      image: "static/images/concentrationcurl.png",
    },
    { name: "Preacher Curls", image: "static/images/preachercurls.png" },
  ];
  ...
  const handleSave = () => {
    ...
    localStorage.setItem("bicepsWorkoutLogs", JSON.stringify(updatedLogs));
    handleCloseModal();
  };
  ...
  const generateReport = () => {
    ...
    toPng(reportRef.current)
      .then((dataUrl) => {
        ...
        link.download = "biceps_workout_report.png";
        link.href = dataUrl;
        link.click();
      })
      .catch((err) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-4 sm:p-6">
      <h1 className="text-2xl font-bold mb-6 mt-4 sm:mt-8">Biceps Exercises</h1>
